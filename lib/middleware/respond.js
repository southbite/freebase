		...
		if (!err){
			...
			if (['PUT','DELETE'].indexOf(req.method) > -1){
				...
				this.publish(req, function(e){
					...
					//_this.streamResults(req, res, status, published_status);
	...
	streamResults:function(req, res, status, published){
		...
		try{
			...
			if (status == 'ok' && req.result){
				...
				res.write('{"status":"ok", "published":"' + published.toString() + '", "data":[');

				console.log('written header');
				console.log('{"status":"ok", "published":"' + published.toString() + '", "data":[');
			...
			res.end(JSON.stringify({status:'error', data:e.toString(), published:false}));
		}
