...
module.exports = {
    initialize:function(config, done){
        ...
        if (config.mode == 'embedded'){
            ...
            if (config.embedded_filename){

                _this.db = new Datastore({ filename: config.embedded_filename });
        ...
        if (message.action == 'GET'){
            ...
            try{
                ...
                console.log(message);
                 ////console.log(fields);
                ////console.log(sort);
                //{$and:[{path:{ $regex: message.path }},criteria]}
                if (message.path.indexOf('*') > 0)
                 ...
                 console.log('in post after find');
                 console.log(cursor);
            ...
            var regularPUT = function(){
                ...
                var upsert = function(){
                    ...
                    _this.db.update({path:message.path}, {$set: setData}, {upsert:true}, function(err, updatedCount) {
                        if (!err){
                            ...
                            if (!dataWasMerged && !params.tag){ // no prefetched object, and we dont need to tag - we need to fetch the object
                                ...
                                _this.getOneByPath(message.path, function(e, updated){
